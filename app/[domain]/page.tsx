...
export function generateMetadata({ params }: { params: { domain: string } }) {
  ...
  return {
    title: `${domain} - obtenha seu usuário para Bluesky`,
    description: `obtenha seu próprio ${domain} handle`,
  }
}
    ...
    <main className="container grid items-center gap-6 pb-8 pt-6 md:py-10">
      <div className="flex max-w-[980px] flex-col items-start gap-4">
        <h1 className="text-3xl font-extrabold leading-tight tracking-tighter sm:text-3xl md:text-5xl lg:text-6xl">
          Obtenha seu próprio {domain} <br className="hidden sm:inline" />
          handle para Bluesky
        </h1>
      ...
      <div>
            ...
            <div className="grid w-full max-w-sm items-center gap-1.5">
              ...
              {error1 && (
                <p className="flex flex-row items-center gap-2 text-sm text-red-500">
                  <X className="size-4" /> usuário não encontrado - por favor tente novamente
                </p>
              )}
              {profile && (
                <>
        ...
        <Stage title="Escolha seu novo usuário" number={2} disabled={!profile}>
          <form>
            ...
            <div className="grid w-full max-w-sm items-center gap-1.5">
              ...
              <p className="text-sm text-muted-foreground">
                Insira o usuário {domain} que você gostaria de ter, sem incluir o @
              </p>
              {error2 && (
                <p className="text-sm text-red-500">
                  {(() => {
                    switch (error2) {
                      ...
                      case "reserved":
                        return "Usuário reservedo - por favor, insira um usuário diferente"
                      default:
              ...
              {success && (
                <p className="text-muted-forground mt-4 flex flex-row items-center gap-2 text-sm">
                  <Check className="size-4 text-green-500" /> Criado com sucesso
                </p>
              )}
            </div>
          </form>
        ...
        >
          ...
          Vá em Configurações {">"} Avançado  {">"} Alterar Usuário. 
           Selecione &quot;Eu tenho meu próprio domínio&quot; e insira{" "}
            {newHandle ? `"${newHandle}"` : "seu novo usuário"}. Por fim, clique
            &quot;Verificar registro DNS&quot;.
          </p>
          <p className="mt-6 max-w-lg text-sm">
            Se você gostou deste projeto, considere apoiar projeto do autor {" "}
            <a href="https://github.com/sponsors/mozzius" className="underline">
              @mozzius
            </a>
            .
            <br/>
            <small>host por <a href="https://bsky.app/profile/chaewon.fearnot.social" className="underline">
              @chaewon.fearnot.social
            </a></small>
          </p>
        </Stage>
      </div>
